...
#include "Gray/Viewer/GlutRenderer.h"
...
void GlutRenderer::RenderViewable(ViewableBase const * const ptr)
{
    ...
    throw(std::runtime_error("Viewable Type rendering not implemented"));
    return;
}
...
void GlutRenderer::RenderViewableCone( const ViewableCone& object )
{
    ...
    for ( i=0; i<=MeshCount; i++ ) {
        double theta = (i<MeshCount) ? (2 * M_PI*i)/MeshCount : 0.0;
...
void GlutRenderer::RenderViewableCylinder( const ViewableCylinder& object )
{
    ...
    // Set material properties for base of cylinder (overriding previous?)
    SetFrontMaterial(object.GetMaterialOuter());
    SetBackMaterial(object.GetMaterialInner());
    ...
    // Set material properties for top of cylinder (overriding previous?)
    SetFrontMaterial(object.GetMaterialOuter());
    SetBackMaterial(object.GetMaterialInner());

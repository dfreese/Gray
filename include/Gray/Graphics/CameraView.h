...
public:
    ...
    const VectorR3& GetCameraPosition() const
    {
        return GetPosition();
    }
    const VectorR3& GetCameraDirection() const
    {
        return GetDirection();
    }

    const VectorR3& GetScreenCenter() const
    {
        return ScreenCenter;
    }

    // The "PixeldU" is the vector from a pixel to it right neighboring pixel
    const VectorR3& GetPixeldU() const
    {
        return pixeldU;
    }
    // The "PixeldV" is the vector from a pixel up to the pixel above it.
    const VectorR3& GetPixeldV() const
    {
        return pixeldV;
    }
    ...
    double GetNearClippingDist() const
    {
        return NearClippingDist;
    }
    double GetFarClippingDist() const
    {
...
inline CameraView::CameraView()
{
    ...
    NearClippingDist = 0.1*ScreenDistance;	// Default near clipping distance
    FarClippingDist = 10.0*ScreenDistance;	// Default far clipping distance
...
inline void CameraView::SetScreenDistance( double dist )
{
    ...
    NearClippingDist = 0.1*dist;	// Default near clipping distance
    FarClippingDist = 10.0*dist;	// Default far clipping distance
    CalcScreenCenter();
}

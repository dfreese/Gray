...
read density
rm -f gridfile_keV.txt
for i in {0..150}; do  echo "11+$i*10" | bc >>gridfile_keV.txt; done ;
rm -f gridfile_XCOM_MeV.txt
echo "151" > gridfile_XCOM_MeV.txt
#echo "61" > $material".dat"
for i in {0..150}; do
    echo "0.011+$i*0.010" | bc >>gridfile_XCOM_MeV.txt;
done
rm -f gridfile_keV_cslevin.txt
for i in `cat  gridfile_keV.txt`; do
  ../CSLEVIN_INTERPOLATE/cslevin_interpolate  $i >> gridfile_keV_cslevin.txt;
done
...
rm -f gridfile_keV_K_cslevin.txt
for i in `cat  $K_grid` ; do
    ../CSLEVIN_INTERPOLATE/cslevin_interpolate $i >> gridfile_keV_K_cslevin.txt
done
...
cat $basefile $Kfile | sort -n >> $material".dat"

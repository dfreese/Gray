...
void LoadDetector::ApplyRotation(const VectorR3& axis, double theta) {
	...
	t2 *= t1;
	cur.m14 = t2.m41; cur.m24 = t2.m42; cur.m34 = t2.m43;
	...
	while ( true ) {
		...
		switch ( cmdNum ) {
			...
			case 4: // from
				...
				if (scanCode != 3) parseErrorOccurred = true;
			break;
			case 5: // lookat
			...
			case 7: // angle
				...
				if (scanCode != 1) parseErrorOccurred = true;
				fovy *= PI/180.0;
			break;
			case 8: // hither
			...
			case 9: // resolution
				...
				if (scanCode != 2) parseErrorOccurred = true;
			break;
			case 10: // light
			{
			...
			case 18: // repeat detector in 3d
			{
				...
				if (scanCode != 9) {
					...
					VectorR3 StartPos;
					StartPos.x = 0.0; StartPos.y = 0.0; StartPos.z = 0.0;
					VectorR3 CurrentPos;
			...
			case 24: // polygon scale
			{
				...
				if (scanCode ==1) polygonScale = t_polygonScale;
				else {
					parseErrorOccurred = true;
					break;
				}
			...
			case 52: // increment detector id, increment before a polygon is called
			{
				...
				VectorR3 UnitSize;
				StartPos.x = 0.0; StartPos.y = 0.0; StartPos.z = 0.0;
				UnitSize.x = 1.0; UnitSize.y = 1.0; UnitSize.z = 1.0;
...
void LoadDetector::ProcessDetector( const VectorR3& detCenter, const VectorR3& detSize, const Material* curMaterial, int id )
{
	...
	vf = vg = vh = detCenter;
	va.x -= sz_tmp.x; va.y -= sz_tmp.y; va.z += sz_tmp.z;
	vb.x += sz_tmp.x; vb.y -= sz_tmp.y; vb.z += sz_tmp.z;
	vc.x -= sz_tmp.x; vc.y += sz_tmp.y; vc.z += sz_tmp.z;
	vd.x -= sz_tmp.x; vd.y -= sz_tmp.y; vd.z -= sz_tmp.z;
	
	ve.x += sz_tmp.x; ve.y += sz_tmp.y; ve.z += sz_tmp.z; // same plane as A C B
	vf.x -= sz_tmp.x; vf.y += sz_tmp.y; vf.z -= sz_tmp.z; // same plane as A C D
	vg.x += sz_tmp.x; vg.y += sz_tmp.y; vg.z -= sz_tmp.z; // same plane as B E H
	vh.x += sz_tmp.x; vh.y -= sz_tmp.y; vh.z -= sz_tmp.z; // same plane as B E G

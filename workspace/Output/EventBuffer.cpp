...
EventBuffer::~EventBuffer()
{
	if (log_singles) singles.close();
	if (log_coincidence) coincidence.close();
...
void EventBuffer::ProcessCoincEvents(Event * e) {	
	...
	if (e->photon.det_id != -1) {
		...
		if (cb.IsEmpty()) {
			existsRed = false;
			existsBlue = false;
			if (e->photon.color == P_RED) existsRed = true; else existsBlue = true;
			cb.Push(e);
		} else {
			// Add events to queue until we have a change in event id
			if (e->photon.id == cb.Top()->photon.id) {
				cb.Push(e);
				if (e->photon.color == P_RED) existsRed = true; else existsBlue = true;
				if (existsRed && existsBlue) isCoinc = true;
			} else {
				// when event id changes, we check to see if we had coincidence
				if (isCoinc) {
					PrintFlushCoinc();
...
bool EventBuffer::AddEvent(const Photon &p, INTER_TYPE type, const MaterialBase & mat, Detector * d) {
	if ((log_singles == false) && (log_coincidence == false)) return false;
	...
	if (log_singles) singles.flush();
	if (log_coincidence) coincidence.flush();
	return true;
}

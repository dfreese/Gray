...
Coincidence::~Coincidence() {
	...
	if (log_coincidence) output.close();
}
...
bool Coincidence::AddEvent(const Photon &p, const INTER_TYPE type, const MaterialBase & mat, Detector * d) {
	...
	if (!mat.GammaProp->log_material) return false;
	if (p.det_id == -1) return false;
	...
	if (coinc.Size() < buffer_size) {
		...
		MergeEventsDetector();
		if (blockEffect) MergeEventsBlock();
		...
		for (long i; coinc.Size() > buffer_limit; i++) {
			...
			bool output_e1;
			Event e1 = NextEvent();
			if (!((e1.photon.energy > energy_gate_lower) && (e1.photon.energy < energy_gate_upper))) continue;

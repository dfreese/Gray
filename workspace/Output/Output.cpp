...
int Output::MakeLogWord(int interaction, int color, bool scatter, int det_mat, int src_id){
  int errorbit=0;
  if (interaction <0 ) { errorbit = 0x80000000 ;interaction*=-1;  } 
...
void Output::LogNuclearDecay(NuclearDecay *p ) {
	if (log_positron) {
		if (binary) LogNuclearDecayBinary(p);
		else LogNuclearDecayASCII(p);
	}
}
...
//	b.rnb = 0;
	b.time = p->time;
	b.energy = p->energy;
	b.x = (float) p->pos.x; b.y = (float) p->pos.y; b.z = (float) p->pos.z;
	b.det_id = -1;
	//	b.m = 0;
	write(b);
}
...
void Output::LogCompton(const Photon &p, double deposit, const MaterialBase & mat) {
	...
	if (log_event) {
	    if (binary) LogBinary(p, COMPTON, deposit, mat);
	    else LogASCII(p, COMPTON, deposit,mat);
...
void Output::LogPhotoElectric(const Photon &p, const MaterialBase & mat) {
		...
		if (log_event) {
		  if (binary) LogBinary(p,PHOTOELECTRIC,p.energy,mat);
		  else LogASCII(p,PHOTOELECTRIC,p.energy,mat);
		}
}
...
void Output::LogASCII(const Photon &p, INTER_TYPE type, double deposit, const MaterialBase & mat) {
	...
	if (!log_all) return;
        if (type == COMPTON ) log_file << " " << 1 << " ";
        else { if (type == PHOTOELECTRIC ) log_file << " " << 3 << " "; 
        else  log_file << " " << 5 << " "; }
	log_file << ptmp;
       	char str[256];
	//	if (log_event) {
...
void Output::LogError(const Photon &p, int t, int det_mat) {
  if (binary) LogErrorBinary(p, t, det_mat);
  else LogErrorASCII(p, t, det_mat);
}
...
void Output::LogErrorBinary(const Photon &p, int t, int detmaterial ) {
	...
	b.time = p.time;
	b.energy = p.energy;
	b.x = p.pos.x; b.y = p.pos.y; b.z = p.pos.z;
	// TODO: code detector ID for binary output
	//	b.det_id = t;
	//	b.m = t;
	write(b);

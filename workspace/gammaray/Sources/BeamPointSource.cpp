...
BeamPointSource::BeamPointSource(
        const VectorR3 &p, const VectorR3 &a, double angle, double energy,
        double act) :
    Source(p, act),
    beam_axis(a),
...
void BeamPointSource::SetIsotope(std::unique_ptr<Isotope> i)
{
    ...
    isotope = std::move(i);
    static_cast<Beam&>(*isotope).SetBeam(beam_axis, beam_angle, beam_energy);
};

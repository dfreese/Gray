...
public:
    ...
    const static int ExtentTripleStorageMultiplier = 2;
...
private:
    long TreeSize() const
    {
        return TreeNodes.SizeUsed();    // Number of nodes in the tree
    }

    Array<KdTreeNode> TreeNodes;
    long RootIndex() const
    {
        return 0;
    }
    long NextIndex()
    ...
    static double DefaultObjectCost()
    {
        return 2.0;
    }
...
public:
    ...
    bool IsLeaf() const
    {
        return (NodeType==KD_LEAF);
    }
    int SplitAxis() const
    {
        assert(NodeType!=KD_LEAF);
        return (int)NodeType;
    }
    bool IsRoot() const
    {
...
public:
    ...
    bool IsMin() const
    {
        return ExtentType==TT_MIN;
    }
    bool IsMax() const
    {
        return ExtentType==TT_MAX;
    }
    bool IsFlat() const
    {
        return ExtentType==TT_FLAT;
    }
...
private:
    ...
    const long NumObjects () const
    {
        return NumMaxMins + NumFlats;
    }
    const long NumTriples () const
    {
        return (NumMaxMins<<1) + NumFlats;
    }
};

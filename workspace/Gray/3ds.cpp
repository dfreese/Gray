...
bool CLoad3DS::Export3DS(t3DModel * pModel, char *strFileName) {
	...
	if (FilePointer != NULL) {
		for(int i=0; i< numobj;i++) {
			 if (pModel->pObject[i].numOfVerts > 0) pr = true;
			 else pr = false;
			 
			 if (pr) fprintf(FilePointer,"const float Object_vert%i[%i][3] = {\n",i,pModel->pObject[i].numOfVerts);
			 for(int j = 0; j < pModel->pObject[i].numOfVerts; j++) {
				...
				fprintf(FilePointer,"\t{%f,%f,%f},\n",
      		   ...
      		   if (pr) fprintf(FilePointer,"};\n",i);
        }
		for(int i=0; i< numobj;i++) {
			  if (pr) fprintf(FilePointer,"const float Object_norm%i[%i][3] = {\n",i,pModel->pObject[i].numOfVerts);
			  for(int j = 0; j < pModel->pObject[i].numOfVerts; j++) {
				//fprintf(FilePointer, "%f\n",pModel->pObjects[i].pVerts[j].x);
				fprintf(FilePointer,"\t{%f,%f,%f},\n",
		...
		for(int i = 0; i < numobj; i++) {
			if (pr) fprintf(FilePointer,"const int Object_tri%i[%i][3] = {\n",i,pModel->pObject[i].numOfFaces);
			...
			pObject->pNormals		= new CVector3 [pObject->numOfVerts];
			  for(int j = 0; j < pObject->numOfVerts; j++) {
				fscanf(FilePointer,"%f %f %f",
     			...
     			if (feof(FilePointer)) printf("ERROR: read past end of file\n");
 			  }
		}
	} else {
